...
export default function AdminAnalyticsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0c1410] text-[#f4f7f2] font-sans">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-10">
        ...
        >
          ...
          <div className="flex items-center gap-2 text-xs text-[#8fa89c] font-mono">
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-xl border border-[#1d2b22] bg-[#10190f] mb-6 sm:mb-8 h-9 sm:h-10 flex items-center"
        >
          ...
          {KPI_CARDS.map((kpi) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-2xl border border-[#1d2b22] bg-[#10190f] p-4 sm:p-5 transition-colors cursor-default"
              >
                <div className="flex items-start justify-between mb-3 sm:mb-4">
                  <span className="flex size-8 sm:size-9 items-center justify-center rounded-lg bg-[#16241a] text-[#3d8a68]">
                    <Icon className="size-4" />
                  </span>
                  <span
                    className={`flex items-center gap-0.5 text-[11px] font-mono font-semibold ${
                      kpi.trend === "up" ? "text-[#3d8a68]" : "text-[#c97b6f]"
                    }`}
                  >
                ...
                <p className="text-xl sm:text-2xl lg:text-3xl font-bold font-mono tracking-tight">
                  <CountUp value={kpi.value} prefix={kpi.prefix || ""} />
                </p>
                <p className="text-[11px] sm:text-xs text-[#8fa89c] mt-1">{kpi.label}</p>
              </motion.div>
            );
          })}
        </motion.div>
        ...
        {/* ══════════════ Charts row ══════════════ */}
        <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-4 sm:gap-5 mb-6 sm:mb-8">
          ...
          <motion.div
            ...
            className="rounded-2xl border border-[#1d2b22] bg-[#10190f] p-4 sm:p-6 transition-colors"
          >
            ...
            <div className="flex items-end gap-1.5 sm:gap-2.5 h-32 sm:h-44">
              {REVENUE_TREND.map((d, i) => (
                <div key={d.month} className="flex-1 flex flex-col items-center gap-1.5 sm:gap-2 group">
                  <motion.div
                    initial={{ height: 0 }}
                  ...
                  >
                    <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] font-mono text-[#a8c4b6] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                      ৳{d.value}K
                    </span>
                  </motion.div>
                  <span className="text-[9px] sm:text-[10px] font-mono text-[#6f8a7c]">{d.month}</span>
                </div>
              ))}
            </div>
          </motion.div>

          {/* Category split — donut-style bars */}
          <motion.div
            initial={{ opacity: 0, y: 24 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="space-y-3.5 sm:space-y-4">
              {CATEGORY_SPLIT.map((cat, i) => (
                <div key={cat.label}>
                  ...
                  <div className="h-1.5 sm:h-2 rounded-full bg-[#16241a] overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
        ...
        <motion.div
          ...
          className="rounded-2xl border border-[#1d2b22] bg-[#10190f] p-4 sm:p-6"
        >
          <div className="flex items-center justify-between mb-4 sm:mb-5">
            <h3 className="text-sm sm:text-base font-semibold">Top Sellers</h3>
            <span className="text-[10px] sm:text-[11px] font-mono text-[#6f8a7c] flex items-center gap-1">
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-[#1d2b22] text-[10px] sm:text-[11px] uppercase tracking-wider text-[#6f8a7c] font-mono">
                  ...
                  <th className="py-2.5 pr-4 font-medium text-right">Growth</th>
                </tr>
              </thead>
              <tbody>
                {TOP_SELLERS.map((s, i) => (
                  ...
                  >
                    <td className="py-3 pr-4 font-mono text-[#6f8a7c]">{String(s.rank).padStart(2, "0")}</td>
                    <td className="py-3 pr-4 font-medium text-[#dce8e0]">{s.name}</td>
                    <td className="py-3 pr-4 font-mono text-[#a8c4b6]">{s.sales}</td>
                    <td className="py-3 pr-4 font-mono text-[#8fa89c]">{s.orders}</td>
                    <td className="py-3 pr-4 text-right">
                      <span className={`inline-flex items-center gap-0.5 font-mono text-xs font-semibold ${
                        s.growth >= 0 ? "text-[#3d8a68]" : "text-[#c97b6f]"
                      }`}>
          ...
          <div className="sm:hidden flex flex-col gap-2.5">
            {TOP_SELLERS.map((s, i) => (
              <motion.div
                ...
                transition={{ delay: 0.55 + i * 0.06, duration: 0.4 }}
                whileHover={{ backgroundColor: "#16241a" }}
                className="rounded-xl border border-[#1d2b22] p-3.5 flex items-center gap-3"
              >
                <span className="font-mono text-[#6f8a7c] text-xs flex-shrink-0">
                  {String(s.rank).padStart(2, "0")}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[#dce8e0] truncate">{s.name}</p>
                  <p className="text-[11px] font-mono text-[#8fa89c]">{s.orders} orders</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-sm font-mono text-[#a8c4b6]">{s.sales}</p>
